...
/* enums */
enum { SchemeNorm, SchemeFade, SchemeHighlight, SchemeHover, SchemeSel, SchemeOut, SchemeGreen, SchemeYellow, SchemeBlue, SchemePurple, SchemeRed, SchemeNormHighlight, SchemeSelHighlight, SchemeLast }; /* color schemes */
...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			...
			x = info[i].x_org + ((info[i].width + dmx  - mw) / 2);
